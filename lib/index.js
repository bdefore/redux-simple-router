...
function syncReduxAndRouter(history, store) {
  ...
  if (!getRouterState()) {
    throw new Error('Cannot sync router: route state does not exist (`state.routing` by default). ' + 'Did you install the routing reducer?');
  }

  var unsubscribeHistory = history.listen(function (location, state) {
  ...
  var unsubscribeStore = store.subscribe(function () {
    ...
    if (lastRoute.changeId !== routing.changeId || !locationsAreEqual(lastRoute, routing)) {
      ...
      lastRoute = routing;
      var method = routing.replace ? 'replace' : 'push';
      history[method]({
